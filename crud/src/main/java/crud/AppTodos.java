...
public class AppTodos {
	...
	private static void listarClientes() {
		try {
			...
			System.out.println("\n📋 LISTA DE CLIENTES:");
			clientes.forEach(
					c -> System.out.printf("ID: %d | Nome: %s | CPF: %s%n", c.getIdcliente(), c.getNome(), c.getCpf()));
	...
	private static void buscarCliente() {
		...
		try {
			...
			if (c != null)
				System.out.println("Encontrado: " + c.getNome() + " - " + c.getEmail());
			else
	...
	private static void listarProdutos() {
		try {
			produtoDAO.listarProdutos().forEach(p -> System.out.printf("ID: %d | Nome: %s | Preço: R$%.2f | Qtd: %d%n",
	...
	private static void atualizarProduto() {
		...
		try {
			// Como seu ProdutoDAO não tem buscarPorId, listamos e filtramos ou pedimos
			// dados novos
			Produto p = new Produto();
